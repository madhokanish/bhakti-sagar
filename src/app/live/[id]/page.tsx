...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${mandir.displayName} Live Darshan`,
    description: `Watch ${mandir.displayName} live darshan, latest livestream recording, or recent uploads from ${mandir.location}.`,
    pathname: `/live/${mandir.slug}`
  });
}
...
export default async function LiveMandirPage({ params }: { params: { id: string } }) {
  ...
  const incomingId = decodeURIComponent(params.id).toLowerCase();
  if (incomingId !== mandir.slug.toLowerCase()) {
    permanentRedirect(`/live/${mandir.slug}`);
  }

  const channelId = mandir.channelId ?? null;
  const moreMandirs = resolvedMandirs
    .filter((item) => item.slug !== mandir.slug)
    .slice(0, 4);
            ...
            <div className="mt-3 space-y-3">
              {moreMandirs.map((item) => (
                ...
                >
                  ...
                  <span>
                    <span className="block text-sm font-semibold text-sagar-ink">
                      {item.displayName}
                    </span>
