...
function buildModeDirective(params: {
  ...
  if (params.mode === "wisdom") {
    ...
    if (isDiscovery) {
      return `Mode=wisdom Strategy=understand_then_hold. This is a personal problem and you do not yet know its specifics. In 2 to 4 sentences total: one short line of real acknowledgment or one guide-colored framing line that only this guide would say, then ask one or two concrete questions about their actual situation, in the same sentence or the next. Ask about facts you genuinely need: how long, who else is involved, what they already tried, what the numbers are. Do not ask vague inward questions like "what does your heart say". Do not deliver the full teaching yet and do not give a generic action. If their message already contains enough detail to answer properly, skip the questions and give your honest read instead. ${getGuideModeFlavor(params.guideId, params.mode)}`;
    }
