  ...
  return (
    <section className="space-y-2">
      <div className="sticky top-24 z-10 rounded-2xl border border-sagar-amber/20 bg-white px-3 py-2">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-sagar-rose">{labels.day_label}</p>
        <p className="text-xs text-sagar-ink/60">
          {dateLabel} · {labels.sunrise} {sunrise ? formatTime(sunrise, timeZone) : "--"} · {labels.sunset}{" "}
      ...
      <div className="space-y-2">
        ...
        {!segments.length && <p className="text-sm text-sagar-ink/60">{labels.select_city_day}</p>}
      </div>
    </section>
  );
}
