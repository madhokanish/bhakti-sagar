...
export function generateStaticParams() {
  return DEITY_TOPICS.krishna.map((topic) => ({ topic }));
}

export function generateMetadata({ params }: { params: { topic: string } }): Metadata {
  const locale = getRequestLanguage();
  return buildTopicMetadata("krishna", params.topic, locale);
}

export default function KrishnaTopicPage({ params }: { params: { topic: string } }) {
  const locale = getRequestLanguage();
  return <DeityTopicTemplate deity="krishna" topic={params.topic} locale={locale} />;
}
